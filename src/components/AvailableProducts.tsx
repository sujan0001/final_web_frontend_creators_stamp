...
export default function AvailableProducts() {
  ...
  const handleRequestTransfer = async (productId: string) => {
    ...
    try {
      ...
      alert("Purchase request sent to creator!");
      await loadProducts(); // optional if you want to refresh state
    } catch (err) {
      console.error("Transfer request failed", err);
      alert("Failed to send request. Make sure you're logged in as a consumer.");
    } finally {
